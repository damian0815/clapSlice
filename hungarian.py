...
def hungarian_algorithm(mat: Tensor) -> Tensor:
    ...
    with tqdm(total=dim) as pbar:
        while zero_count < dim:
            ans_pos, marked_rows, marked_cols = mark_matrix(cur_mat)
            new_zero_count = len(marked_rows) + len(marked_cols)
            pbar.update(new_zero_count - zero_count)
            if zero_count == new_zero_count and zero_count < dim:
                print(f"stuck with {dim-zero_count}/{dim} non-zero entries")
                break
